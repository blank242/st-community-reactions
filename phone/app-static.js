    ...
    for (const meta of STATIC_APPS) {
        registry.registerApp(function createStaticPhoneApp(context) {
            ...
            function renderHomeIcon(app) {
                ...
                const actionAttribute = meta.action
                    ? `data-phone-action="${escapeHtml(meta.action)}"${meta.id === 'generate' ? ` data-phone-app="${escapeHtml(app.app_id)}"` : ''}`
        ...
        `;
            ...
            function renderApp(app) {
                if (meta.id === 'generate') {
                    return `
        <section class="crx-phone-app crx-phone-composer-app" data-phone-app="${escapeHtml(app.app_id)}">
            <div class="crx-phone-composer-placeholder">
                <span>${escapeHtml(app.app_home_label)}</span>
            </div>
        </section>
    `;
