...
예를 들어 선택한 채팅 구간을 소설의 한 장면, 드라마 에피소드, 웹툰 회차, 영화 장면, 게임 이벤트처럼 취급한 뒤 Twitter/X, 카페, 에브리타임, 웹소설 리뷰 페이지 같은 형태의 반응을 생성합니다.
...
3. 휴대폰 홈에서 원하는 앱을 선택합니다.
4. 선택한 앱에 해당하는 커뮤니티/카테고리의 저장된 반응을 확인합니다.
5. '생성하기'에서는 반응 유형, 커뮤니티, 메시지 범위, 국가, 출력 언어, 게시글 수 등을 설정한 뒤 `생성하기`를 누릅니다.
